...
function run_Antarctica(;itMax=2*10^4, make_plot=false,
    dt=1e9, tsteps=1, γ_ϕ= 0.9, γ_h=0.8, dτ_ϕ_=1.0, dτ_h=6e-6, thin=10)      # parameters for pseudo-transient time stepping
    ...
    # call the SheetModel
    input = make_model_input(H, zb, Lx, Ly, dx, dy, ttot, dt, itMax, γ_ϕ, γ_h, dτ_ϕ_, dτ_h, ϕ_init, h_init, calc_m, ice_mask, bc_diric, bc_no_xflux, bc_no_yflux)
...
input, output = run_Antarctica(γ_ϕ=0.81, γ_h=0.86, dτ_ϕ_=8e-7, dτ_h=1e-20, itMax=100*10^3, thin=50);
ice = input.ice_mask
