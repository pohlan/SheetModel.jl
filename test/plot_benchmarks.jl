...
for (n, unit) in enumerate(keys(benchmarks))
    ...
    case = benchmarks[unit][gitcommit][:test_case]
    ...
    if n == 1
        ...
        global plt_Teff = scatterplot(res[.!steadyst], T_eff[.!steadyst], marker=symbols[case], color=colors[n], name=unit, xlabel="Grid size (nx*ny)", ylabel="T_eff (GB/s)")
    else
        #scatterplot!(plt_ttot, res[steadyst], t_tot[steadyst], marker=symbols[host], color=colors[PU], name=unit)
        scatterplot!(plt_Teff, res[.!steadyst], T_eff[.!steadyst], marker=symbols[case], color=colors[n], name=unit)
    end
end
